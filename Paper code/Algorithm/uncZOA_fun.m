function uncZOA = uncZOA_fun(Popsize, Max_iter, dim, lb, ub, thres, fobj, XTRNf, YTRNf, XTSNf, YTS)
...
uncZOA_Curve = zeros(1, Max_iter);

%%
Initial_Value = 0.65;

x = (chaos(3, Initial_Value, Max_iter));
y = (chaos(3, Initial_Value, Max_iter));
...
for t = 1: Max_iter
    ...
    for i = 1: Popsize
        ...
        I = round(1 + rand);
        X_newP1 = ( 1.8*( 0.5-0.5*(2*(t/Max_iter)-1)^3 ) ) * v(t) * X(i, :) + rand(1, dim) .* (PZ - I.* X(i, :)); %Eq(3)
    ...
    uncZOA_Curve(t) = fbest;
    fprintf('\nIteration %d Best (uncZOA)= %f', t, uncZOA_Curve(t))

end %t=1:Max_iterations

uncZOA.bestscore = fbest;
uncZOA.bestpos = PZ;
uncZOA.curve = uncZOA_Curve;

Pos   = 1: dim;
uncZOA.bestbinarypos = (uncZOA.bestpos > thres) == 1;
Sf    = Pos((PZ > thres) == 1);
uncZOA.sf = Sf;
uncZOA.nf = length(Sf);
