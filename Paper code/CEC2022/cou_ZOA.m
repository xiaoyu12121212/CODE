function [Best_score, Best_pos, couZOA_curve] = cou_ZOA(Popsize, Max_iter, lb, ub, dim, fobj, n1, n2)
...
for t = 1: Max_iter
    ...
    switch n1
        case 1;         x(t+1) = (3*y(t)+1)*x(t)*(1-x(t));
            y(t+1) = (3*x(t)+1)*y(t)*(1-y(t));
        case 2;         x(t+1) = x(t)-0.2*( x(t)^12+y(t)^12-1 );
            y(t+1) = y(t)+2.4*x(t)*y(t)*( x(t)^12+y(t)^12-1 );
        case 3;        x(t+1) = ( sin(4*pi* 0.19 *y(t)*(1-y(t))) +2.9) * x(t)*(1-x(t));
            y(t+1) = ( sin(4*pi* 0.19 *x(t+1)*(1-x(t+1))) +2.9) * y(t)*(1-y(t));
    end
...
for t = 1: Max_iter
    ...
    for i = 1: Popsize
        ...
        I = round(1 + rand);
        X_newP1 = ( 2*( 0.5-0.5*(2*(t/Max_iter)-1)^3 ) ) * v(t) * X(i, :) + rand(1, dim).*(PZ-I.* X(i, :));
